...
class ChatBot:
    def __init__(self):
        ...
        self.module_path = os.path.normpath(
            os.path.dirname(self.script_path.rstrip("/"))
        )
        self.persist_directory = f"{self.module_path}" f"/chromadb"
        self.model_directory = f"{self.module_path}" f"/models"
        ...
        self.model_path = os.path.normpath(
            os.path.join(self.model_directory, self.model)
        )
    ...
    def check_hardware(self):
        ...
        self.used_percent = psutil.virtual_memory()[2]
        logging.info(
            f"RAM Utilization: {round(self.used_percent, 2)}%\n"
    ...
    def chat(self, prompt: str) -> dict:
        ...
        if os.path.isfile(os.path.join(self.model_directory, self.model)):
            ...
            print("Model was not found, downloading...")
            gpt4all.GPT4All.retrieve_model(
                self.model, self.model_directory, allow_download=True
            )
        # Prepare the LLM
        match self.model_engine.lower():
            ...
            case _default:
                ...
                raise Exception(
                    f"Model type {self.model_engine} is not supported. "
        ...
        answer, docs = res["result"], (
            [] if self.hide_source else res["source_documents"]
        )
        end = time.time()
        documents = ""
    ...
    def assimilate(self):
        if self.vectorstore == "chromadb":
            ...
            if self.does_vectorstore_exist():
                ...
                for batched_chromadb_insertion in self.batch_chromadb_insertions(
                    chromadb_client, documents
                ):
                ...
                batched_chromadb_insertions = self.batch_chromadb_insertions(
                    chromadb_client, documents
                )
    ...
    def load_documents(self, source_dir: str, ignored_files: List[str] = List):
        ...
        for ext in self.loader_mapping:
            all_files.extend(
                glob.glob(
                    os.path.join(source_dir, f"**/*{ext.lower()}"), recursive=True
                )
            )
            all_files.extend(
                glob.glob(
                    os.path.join(source_dir, f"**/*{ext.upper()}"), recursive=True
                )
            )
        filtered_files = [
    ...
    def process_documents(self, ignored_files: List[str] = []) -> List[Document]:
        ...
        print(
            f"Split into {len(documents)} chunks of text (max. {self.chunk_size} tokens each)"
        )
        return documents

    @staticmethod
    def batch_chromadb_insertions(
        chromadb_client: SegmentAPI, documents: List[Document]
    ) -> List[Document]:
        # Get max batch size.
...
def usage():
    print(
        ...
        "   | --chromadb-directory [ Directory for chromadb persistent storage ]\n"
        "   | --chunks             [ Number of chunks to use ]\n"
        "-e | --embeddings-model   [ Embeddings model to use https://www.sbert.net/docs/pretrained_models.html ]\n"
        ...
        "-m | --model              [ Model to use from GPT4All https://gpt4all.io/index.html ]\n"
        "   | --max-token-limit    [ Maximum token to generate ]\n"
        "   | --model-directory    [ Directory to store models ]\n"
...
def genius_chatbot(argv):
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            geniusbot_chat.embeddings = HuggingFaceEmbeddings(
            ...
            if (
                geniusbot_chat.model_engine.lower() != "llamacpp"
    ...
    if run_flag:
        ...
        if geniusbot_chat.vectorstore == "chromadb":
            geniusbot_chat.chromadb_client = chromadb.PersistentClient(
                settings=geniusbot_chat.chroma_settings,
